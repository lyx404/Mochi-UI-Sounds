...
[https://mochi-ui-sounds.vercel.app](https://mochi-ui-sounds.vercel.app)

## 中文

Mochi UI Sounds 是一个轻量的 Web Audio UI 音效库，并打包成了通用 AI agent
skill。安装后，你可以让 Codex、Claude Code、OpenClaw 等工具把柔软、轻快的交互音效接入按钮、表单、菜单、
提示、弹窗和其他产品状态。
...
安装完成后，重启对应的 AI agent，让新的 skill 生效。

然后你可以对 Codex 或其他支持 skills 的 agent 说：

```text
Use $mochi-ui-sounds to add playful UI interaction sounds to this app.
```

AI agent 会把内置的 `sounds.ts` 模块安装到你的项目中，并根据项目里的交互场景
接入合适的音效。
...
如果你只使用 Codex、Claude Code 等已成功安装的 agent，可以忽略。只要下面的命令能看到
`mochi-ui-sounds`，就说明 skill 已经可用：
...
这个仓库里也保留了 shadcn registry 实验，但推荐的分发方式是 AI agent skill：
...
Mochi UI Sounds is a tiny Web Audio UI sound library packaged as a portable AI agent skill.
It helps Codex, Claude Code, OpenClaw and other supported agents add soft, playful interaction sounds to buttons, forms, menus,
...
Restart your AI agent after installation so the new skill is picked up.

Then ask Codex or another skills-compatible agent:

```text
Use $mochi-ui-sounds to add playful UI interaction sounds to this app.
```

The AI agent will install the bundled `sounds.ts` module into your project and wire it
...
you can ignore it when using agents that installed successfully, such as Codex or Claude Code.
The skill is available as long as this command lists `mochi-ui-sounds`:
...
distribution path is the AI agent skill:
